...
def rotate_images_voc(path: str, angles: list):
    ...
    for path in labels:
        for a in angles:
            ...
            lbl = np.array(lbl)
            rotated_lbl = rotate_image(lbl, a, cv2.INTER_NEAREST)
    ...
    for path in images:
        for a in angles:
            img = cv2.imread(path)
            rotated_img = rotate_image(img, a, cv2.INTER_LINEAR)
...
def crop_image_voc(path: str, x1: float, y1: float, x2: float, y2: float):
    ...
    for filename in filenames:
        ...
        img.save(os.path.join(processed_dir, os.path.basename(filename)))
