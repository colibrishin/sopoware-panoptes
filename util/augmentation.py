...
def rotate_images(path: str, angles: list):
    '''
    path - Dataset folder that split with train/valid
    ...
    it_root = os.path.join(path, 'images', 'train')
    iv_root = os.path.join(path, 'images', 'valid')
    ...
    for path in labels:
        for a in angles:
            ...
            save_dir, new_name = os.path.split(path)
            ...
            rotated_lbl.save(os.path.join(save_dir, new_name))
    ...
    for path in images:
        for a in angles:
            ...
            cv2.imwrite(os.path.join(save_dir, new_name), rotated_img)
